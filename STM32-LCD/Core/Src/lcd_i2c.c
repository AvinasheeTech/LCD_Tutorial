...
#include "stm32f4xx_hal.h"         //replace with your stm32 mcu headers
...
#define I2C_MASTER_SEND_DATA(...)     (master_send_data(__VA_ARGS__))         //transmit i2c data function (platform dependent)
#define I2C_INIT(...)                 (i2c_init(##__VA_ARGS__))               //i2c initialize function (platform dependent) 

//variables
uint8_t lcd_error_code = 0;
...
void master_send_data(uint8_t data,uint8_t i2c_addr){
	HAL_I2C_Master_Transmit(&hi2c1, (i2c_addr<<1), &data, sizeof(data), 100);
}
